...
public class PagingView
{
    ...
    public void InitColumns(List<string> fieldNames, List<TableColumnView> columnHeads, Type type)
    {
        ...
        {
            var tableInfo = entityInfos.FirstOrDefault(w => w.Name == item);
            var title = string.IsNullOrWhiteSpace(tableInfo?.Remark) ? item : tableInfo.Remark;
            Columns.Add(new TableColumnView(item.FirstCharToLower(), title));
        }
        ...
        if (columnHeads != null)
        {
            ...
            {
                var columnHead = Columns.Find(w => w.FieldName.ToLower() == item.FieldName.ToLower());
