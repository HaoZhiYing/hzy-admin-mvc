...
public class SysMenuService : AdminBaseService<IAdminRepository<SysMenu>>
{
    ...
    public async Task<PagingView> FindListAsync(int page, int size, SysMenu search)
    {
        var query = (from sysMenu in this.Repository.Orm.SysMenu
                ...
                .Select(w => new
                {
                    ...
                    w.t1.IsClose,
                    w.t1.IsShow,
                    UpdateTime = w.t1.UpdateTime.Value.ToString("yyyy-MM-dd"),
    ...
    public async Task<List<SysMenu>> GetMenuByRoleIdAsync()
    {
        ...
        var sysMenuList = await (
                ...
                .Where(w => this._accountInfo.SysRoles.Select(w => w.Id).Contains(w.t1.RoleId))
                .Where(w => w.t2.ByName == AdminFunctionConsts.Function_Display && w.t3.IsShow == 1)
                .Select(w => w.t3)
                .ToListAsync()
            ;
    ...
    public async Task<List<Dictionary<string, object>>> GetPowerState(List<SysMenu> sysMenuList)
    {
        ...
        if (this._accountInfo.IsAdministrator)
        {
            foreach (var item in sysMenuList)
            {
                ...
                foreach (var sysFunction in sysFunctionList)
                {
                    ...
                    if (sysFunction.ByName == AdminFunctionConsts.Function_Display || item.ParentId == this._appConfiguration.Configs.SysMenuId)
                        isPower = true;
        ...
        foreach (var item in sysMenuList)
        {
            ...
            foreach (var sysFunction in sysFunctionList)
            {
                ...
                if (this._accountInfo.SysRoles.Select(w => w.Id).ToList()?.Count > 0)
                {
    ...
    public async Task<Dictionary<string, bool>> GetFindBackPower(string findBack, bool isCheck = true)
    {
        ...
        if (isCheck)
        {
            foreach (var item in sysFunctionList)
            {
                ...
                if (item.ByName == AdminFunctionConsts.Function_Display || item.ByName == AdminFunctionConsts.Function_Search ||
    ...
    public async Task<Dictionary<string, bool>> GetPowerStateByMenuId(Guid menuId)
    {
        ...
        if (this._accountInfo.IsAdministrator)
        {
            foreach (var item in sysFunctionList)
            {
                ...
                if (item.ByName == AdminFunctionConsts.Function_Display || sysMenu.ParentId == this._appConfiguration.Configs.SysMenuId)
                    isPower = true;
